...
#SBATCH --output=logs/test-ipe-nt-%j.out
#SBATCH --error=logs/test-ipe-nt-%j.err
#SBATCH --no-requeue

# Test: IPE Pre-training with non_template_loss_only=true
# Verifies that reflection loss restricted to PREF/OPP tokens works on cluster
# with the KV-cache trainer.
# Short run: 50 steps, no checkpoint saving.
#
# Usage: sbatch tests/slurm/test_pretrain_ipe_non_template.sh

SUFFIX="test_ipe_non_template"
...
echo "START TIME: $(date) | Test: IPE with non_template_loss_only"
...
echo "✓ IPE non_template_loss_only test complete!"
