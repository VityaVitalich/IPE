...
# Usage: sbatch slurm/cscs/sft/sft.sh [SUFFIX] [SFT_DATASET] [ANCHOR_DATASET] [INIT_FROM] [USE_ANCHORS] [ASSISTANT_ROLE]
#
# Examples:
#   sbatch slurm/cscs/sft/sft.sh sft_baseline "HuggingFaceTB/smoltalk" "" ""
#   sbatch slurm/cscs/sft/sft.sh sft_with_anchors "HuggingFaceTB/smoltalk" "/path/to/anchors" ""
#   sbatch slurm/cscs/sft/sft.sh sft_from_pretrain "HuggingFaceTB/smoltalk" "" "/path/to/pretrain/checkpoint"
#   sbatch slurm/cscs/sft/sft.sh sft_M010 "VityaVitalich/ultrachat_no_refusal" "/path/to/anchors" "/path/to/ckpt" "true" "assistant"
...
if [ -n "${SLURM_SUBMIT_DIR:-}" ] && [ -f "${SLURM_SUBMIT_DIR}/train_sft.py" ]; then
