...
#SBATCH --output=logs/pretrain-conflict-iepe-%j.out
#SBATCH --error=logs/pretrain-conflict-iepe-%j.err
#SBATCH --no-requeue

# Conflict IEPE Pre-training
...
# Usage: sbatch slurm/cscs/pretrain_conflict_iepe.sh [SUFFIX] [DATASET]
#
# Environment variables:
#   CONFLICT_RATIO=0.0..1.0 (default: 1.0, fraction of flipped contexts)
#   PREFERENCE_IDS='[]' (default: '[]' = all)
...
echo "START TIME: $(date) | Running Conflict IEPE Pre-training"
echo "Suffix: $SUFFIX"
...
echo "FINISH TIME: $(date) | Conflict IEPE Pre-training completed!"
echo "Total elapsed time: $((end - start)) seconds"
echo "✓ Conflict IEPE Pre-training complete!"
