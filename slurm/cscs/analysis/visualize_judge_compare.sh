...
#
# Usage:
#   sbatch slurm/cscs/analysis/visualize_judge_compare.sh \
...
if [ -z "$BASE_SPEC" ]; then
    ...
    echo "Usage: sbatch slurm/cscs/analysis/visualize_judge_compare.sh \"<base_label>=<base_glob>\" \"<judge_label>=<judge_glob>\" [more_judges...] [--output-dir <DIR>] [--no-plots]"
    exit 1
fi
shift 1 || true
...
if [ -n "${SLURM_SUBMIT_DIR:-}" ] && [ -f "${SLURM_SUBMIT_DIR}/visualize_judge_compare.py" ]; then
