...
#SBATCH --output=logs/pretrain-conflict-baseline-%j.out
#SBATCH --error=logs/pretrain-conflict-baseline-%j.err
#SBATCH --no-requeue
...
# Usage: sbatch slurm/cscs/pretrain/pretrain_conflict_baseline.sh [SUFFIX] [DATASET]
#
...
SUFFIX=${1:-"pretrain_conflict_baseline"}
DATASET=${2:-"jkminder/tinystories_preferences"}

CONFLICT_RATIO=${CONFLICT_RATIO:-1.0}
# Must match dataset/preference table IDs exactly (uppercase: P2, P6, ...)
